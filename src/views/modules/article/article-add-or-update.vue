  ...
  <div>
    <el-form :model="article" label-width="80px" :rules="rules" ref="articleForm">
      ...
      <!-- <el-form-item label="上传封面">
        <el-col :span="12">
          <el-upload
            ...
            :file-list="file"
            :headers="{authorization:'Bearer ' +(this.$cookie.get('token')||localStorage.getItem('token'))}"
      ...
      <u-editor ref="editor"  :height="400" :html="html" />
      </el-form-item>
      <el-form-item >
        <el-button type="primary" @click="saveArticle()">保存</el-button>
        <!-- <el-button >重置</el-button> -->
        <el-button @click="prviewOpen">预览</el-button>
      </el-form-item>
    </el-form>
    <el-dialog  :visible.sync="prviewVis" title="预览内容" top="6vh">
...
export default {
  ...
  methods: {
        ...
        if (id) {
          this.$http({
            ...
            if (data && data.code === 200) {
              ...
              // this.html=data.article.contentFormat
              this.$refs.editor.setContent(data.article.contentFormat)
    ...
    saveArticle () {
        ...
        var content=this.$refs.editor.getContent()
        if(content&&content.split('src="').length!=1){
            this.article.cover=content.split('src="')[1].split('"')[0]
        }
      this.article.content = content
        this.article.contentFormat=content
