...
export default {
  ...
  mounted() {
    this.ueditor = UE.getEditor(this.id, { ...config })
      console.log(this.html,'this.ueditor')
      // if (this.html) {
  ...
  methods: {
    ...
    init() {
      this.ueditor = UE.getEditor(this.id, { ...config })
      console.log(this.ueditor,'this.ueditor')
      // if (this.html) {
      //   this.loadUrl(this.html)
      // }
      this.editor.addListener("ready", ()=> {
    ...
    // 图片上传成功添加到编辑器
    async imageSuccess(arg) {
      const {urlList,attr}=arg
      console.log(attr)
      var pHeight
      var pWidth

      if(attr.pHeight){
        pHeight=attr.pHeight.replace(/px/g, "")
        pHeight+='px'
      }else{
        pHeight='100%'
      }
      if(attr.pWidth){
        pWidth=attr.pWidth.replace(/px/g, "")
        pWidth+='px'
      }else{
        pWidth='100%'
      ...
      try {
        ...
        urlList.forEach(item => {
          const image = `<img width="${pWidth}" height="${pHeight}" src="${item.data.resource.url}">`
    ...
    setContent(content, isAppendTo) {
      console.log(content,'传递来的content')
      if (!content) return
      this.ueditor.ready(() => {
